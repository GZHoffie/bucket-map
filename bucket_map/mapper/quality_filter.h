#ifndef BUCKET_MAP_QUALITY_FILTER
#define BUCKET_MAP_QUALITY_FILTER
...
{
// ---------------------------------------------------------------------------------------------------------------------
// kmer_quality_view class
// ---------------------------------------------------------------------------------------------------------------------

/*!\brief The type returned by seqan3::views::kmer_quality.
 * \tparam urng_t The type of the underlying ranges, must model std::forward_range, the reference type must model
 *                seqan3::semialphabet. Should be indicating the quality.
...
class kmer_quality_view : public std::ranges::view_interface<kmer_quality_view<urng_t>>
{
private:
    static_assert(std::ranges::forward_range<urng_t>, "The kmer_quality_view only works on forward_ranges");
...
public:
    ...
    = default;                                                        //!< Defaulted.
    kmer_quality_view(kmer_quality_view const & rhs) = default;             //!< Defaulted.
    kmer_quality_view(kmer_quality_view && rhs) = default;                  //!< Defaulted.
    kmer_quality_view & operator=(kmer_quality_view const & rhs) = default; //!< Defaulted.
    kmer_quality_view & operator=(kmer_quality_view && rhs) = default;      //!< Defaulted.
    ~kmer_quality_view() = default;                                      //!< Defaulted.
    ...
    template <typename rng_t>
        requires (!std::same_as<std::remove_cvref_t<rng_t>, kmer_quality_view>) && std::ranges::viewable_range<rng_t>
                  && std::constructible_from<urng_t, std::ranges::ref_view<std::remove_reference_t<rng_t>>>
    explicit kmer_quality_view(rng_t && urange_, shape const & s_) :
...
template <std::ranges::view urng_t>
template <bool const_range>
class kmer_quality_view<urng_t>::basic_iterator :
...
public:
    ...
    */
    ...
    //!\anchor basic_iterator_comparison_kmer_quality
...
kmer_quality_view(rng_t &&, shape const & shape_) -> kmer_quality_view<std::views::all_t<rng_t>>;

// ---------------------------------------------------------------------------------------------------------------------
// kmer_quality_fn (adaptor definition)
// ---------------------------------------------------------------------------------------------------------------------

//![adaptor_def]
//!\brief views::kmer_quality's range adaptor object type (non-closure).
//!\ingroup search_views
struct kmer_quality_fn
{
    ...
    constexpr auto operator()(urng_t && urange, shape const & shape_) const
    {
        static_assert(std::ranges::viewable_range<urng_t>,
                      "The range parameter to views::kmer_quality cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng_t>,
                      "The range parameter to views::kmer_quality must model std::ranges::forward_range.");
        static_assert(semialphabet<std::ranges::range_reference_t<urng_t>>,
                      "The range parameter to views::kmer_quality must be over elements of seqan3::semialphabet.");

        return kmer_quality_view{std::forward<urng_t>(urange), shape_};
    }
};
//![adaptor_def]
 ...
 */
inline constexpr auto kmer_quality = detail::kmer_quality_fn{};
