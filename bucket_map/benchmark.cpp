...
int main() {
    ...
    seqan3::shape bucket_shape(0b111111111_shape);
    ...
    q_gram_mapper<26507> map(bucket_length, read_length, bucket_shape, 40, 4, 0.5);
    bucket_locator loc(&ind, &map, bucket_length, read_length, locate_shape, 0.02 * locate_shape.count(), 0.001 * std::ranges::size(locate_shape), 40);
