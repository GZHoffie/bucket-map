...
int main() {
    ...
    seqan3::shape bucket_shape(0b111010010100110101_shape);

    int bucket_length = 262144;
    int read_length = 150;

    bucket_fm_indexer<7139> ind(bucket_length, read_length, bucket_shape);
    q_gram_mapper<7139> map(bucket_length, read_length, bucket_shape, 50, 10, 0.5);

    //ind.index(genome_file, data_path / "index_10-mer");
    ...
    //sim.generate_fastq_file(data_path / "test", "sim_10-mer", 1000000);

    auto res = map._query_file(data_path / "test" / "sim_10-mer.fastq");
    map._check_ground_truth(res, data_path / "test" / "sim_10-mer.bucket_ground_truth");
